...
void Viewport::set_canvas_transform(Transform2D p_xform){
	_wasgo_Viewport_wrapper_set_canvas_transform(wasgo_id, ((Variant) xform).get_wasgo_id());
}
void Viewport::set_global_canvas_transform(Transform2D p_xform){
	_wasgo_Viewport_wrapper_set_global_canvas_transform(wasgo_id, ((Variant) xform).get_wasgo_id());
}
void Viewport::set_size(Vector2 p_size){
	_wasgo_Viewport_wrapper_set_size(wasgo_id, ((Variant) size).get_wasgo_id());
}
void Viewport::set_transparent_background(bool p_enable){
	_wasgo_Viewport_wrapper_set_transparent_background(wasgo_id, enable);
}
void Viewport::set_use_arvr(bool p_use){
	_wasgo_Viewport_wrapper_set_use_arvr(wasgo_id, use);
}
void Viewport::set_world(World p_world){
	_wasgo_Viewport_wrapper_set_world(wasgo_id, ((Variant) world).get_wasgo_id());
}
void Viewport::set_world_2d(World2D p_world_2d){
	_wasgo_Viewport_wrapper_set_world_2d(wasgo_id, ((Variant) world_2d).get_wasgo_id());
}
bool Viewport::use_arvr(){
	return (bool) _wasgo_Viewport_wrapper_use_arvr(wasgo_id));
}

Viewport::Viewport(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
Viewport::~Viewport(){
}