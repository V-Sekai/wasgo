...
void Timer::set_autostart(bool p_enable){
	_wasgo_Timer_wrapper_set_autostart(wasgo_id, enable);
}
void Timer::set_one_shot(bool p_enable){
	_wasgo_Timer_wrapper_set_one_shot(wasgo_id, enable);
}
void Timer::set_paused(bool p_paused){
	_wasgo_Timer_wrapper_set_paused(wasgo_id, paused);
}
void Timer::set_timer_process_mode(Timer::TimerProcessMode p_mode){
	_wasgo_Timer_wrapper_set_timer_process_mode(wasgo_id, ((Variant) mode).get_wasgo_id());
}
void Timer::set_wait_time(float p_time_sec){
	_wasgo_Timer_wrapper_set_wait_time(wasgo_id, time_sec);
}
void Timer::start(float p_time_sec = (float) -1){
	_wasgo_Timer_wrapper_start(wasgo_id, time_sec);
}
void Timer::stop(){
	_wasgo_Timer_wrapper_stop(wasgo_id);
}

Timer::Timer(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
Timer::~Timer(){
}