...
Error StreamPeerSSL::accept_stream(StreamPeer p_stream, CryptoKey p_private_key, X509Certificate p_certificate, X509Certificate p_chain = (X509Certificate) [object:null]){
	return Error::from_wasgo_id(_wasgo_StreamPeerSSL_wrapper_accept_stream(wasgo_id, ((Variant) p_stream).get_wasgo_id(), ((Variant) p_private_key).get_wasgo_id(), ((Variant) p_certificate).get_wasgo_id(), ((Variant) p_chain).get_wasgo_id()));
}
Error StreamPeerSSL::connect_to_stream(StreamPeer p_stream, bool p_validate_certs = (bool) false, String p_for_hostname = (String) , X509Certificate p_valid_certificate = (X509Certificate) [object:null]){
	return Error::from_wasgo_id(_wasgo_StreamPeerSSL_wrapper_connect_to_stream(wasgo_id, ((Variant) p_stream).get_wasgo_id(), p_validate_certs, ((Variant) p_for_hostname).get_wasgo_id(), ((Variant) p_valid_certificate).get_wasgo_id()));
}
