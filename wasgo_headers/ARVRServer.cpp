...
void ARVRServer::center_on_hmd(ARVRServer::RotationMode p_rotation_mode, bool p_keep_height){
	_wasgo_ARVRServer_wrapper_center_on_hmd(wasgo_id, ((Variant) rotation_mode).get_wasgo_id(), keep_height);
}
ARVRInterface ARVRServer::find_interface(String p_name){
	return ARVRInterface::from_wasgo_id(_wasgo_ARVRServer_wrapper_find_interface(wasgo_id, ((Variant) name).get_wasgo_id()));
}
...
ARVRInterface ARVRServer::get_interface(int p_idx){
	return ARVRInterface::from_wasgo_id(_wasgo_ARVRServer_wrapper_get_interface(wasgo_id, idx));
}
...
ARVRPositionalTracker ARVRServer::get_tracker(int p_idx){
	return ARVRPositionalTracker::from_wasgo_id(_wasgo_ARVRServer_wrapper_get_tracker(wasgo_id, idx));
}
...
void ARVRServer::set_primary_interface(ARVRInterface p_interface){
	_wasgo_ARVRServer_wrapper_set_primary_interface(wasgo_id, ((Variant) interface).get_wasgo_id());
}
void ARVRServer::set_world_scale(float p_arg0){
	_wasgo_ARVRServer_wrapper_set_world_scale(wasgo_id, arg0);
}

ARVRServer::ARVRServer(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
