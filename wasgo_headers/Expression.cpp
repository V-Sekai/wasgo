...
Variant Expression::execute(Array p_inputs = (Array) [], Object p_base_instance = (Object) "", bool p_show_error = (bool) true){
	return Variant::from_wasgo_id(_wasgo_Expression_wrapper_execute(wasgo_id, ((Variant) p_inputs).get_wasgo_id(), ((Variant) p_base_instance).get_wasgo_id(), p_show_error));
}
...
Error Expression::parse(String p_expression, PoolStringArray p_input_names = (PoolStringArray) []){
	return Error::from_wasgo_id(_wasgo_Expression_wrapper_parse(wasgo_id, ((Variant) p_expression).get_wasgo_id(), ((Variant) p_input_names).get_wasgo_id()));
}