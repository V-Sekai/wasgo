...
Error HashingContext::start(HashingContext::HashType p_type){
	return Error::from_wasgo_id(_wasgo_HashingContext_wrapper_start(wasgo_id, ((Variant) p_type).get_wasgo_id()));
}
Error HashingContext::update(PoolByteArray p_chunk){
	return Error::from_wasgo_id(_wasgo_HashingContext_wrapper_update(wasgo_id, ((Variant) p_chunk).get_wasgo_id()));
}