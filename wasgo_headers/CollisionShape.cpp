...
void CollisionShape::resource_changed(Resource p_resource){
	_wasgo_CollisionShape_wrapper_resource_changed(wasgo_id, ((Variant) p_resource).get_wasgo_id());
}
...
void CollisionShape::set_shape(Shape p_shape){
	_wasgo_CollisionShape_wrapper_set_shape(wasgo_id, ((Variant) p_shape).get_wasgo_id());
}