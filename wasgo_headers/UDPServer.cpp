...
Error UDPServer::listen(int p_port, String p_bind_address = (String) *){
	return Error::from_wasgo_id(_wasgo_UDPServer_wrapper_listen(wasgo_id, port, ((Variant) bind_address).get_wasgo_id()));
}
...
void UDPServer::set_max_pending_connections(int p_max_pending_connections){
	_wasgo_UDPServer_wrapper_set_max_pending_connections(wasgo_id, max_pending_connections);
}
void UDPServer::stop(){
...
UDPServer::UDPServer(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
UDPServer::~UDPServer(){
}