...
Mesh Mesh::create_outline(float p_margin){
	return Mesh::from_wasgo_id(_wasgo_Mesh_wrapper_create_outline(wasgo_id, margin));
}
...
void Mesh::set_lightmap_size_hint(Vector2 p_size){
	_wasgo_Mesh_wrapper_set_lightmap_size_hint(wasgo_id, ((Variant) size).get_wasgo_id());
}
Array Mesh::surface_get_arrays(int p_surf_idx){
	return Array::from_wasgo_id(_wasgo_Mesh_wrapper_surface_get_arrays(wasgo_id, surf_idx));
}
Array Mesh::surface_get_blend_shape_arrays(int p_surf_idx){
	return Array::from_wasgo_id(_wasgo_Mesh_wrapper_surface_get_blend_shape_arrays(wasgo_id, surf_idx));
}
Material Mesh::surface_get_material(int p_surf_idx){
	return Material::from_wasgo_id(_wasgo_Mesh_wrapper_surface_get_material(wasgo_id, surf_idx));
}
void Mesh::surface_set_material(int p_surf_idx, Material p_material){
	_wasgo_Mesh_wrapper_surface_set_material(wasgo_id, surf_idx, ((Variant) material).get_wasgo_id());
}