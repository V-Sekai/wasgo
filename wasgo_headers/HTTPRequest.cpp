...
Error HTTPRequest::request(String p_url, PoolStringArray p_custom_headers = (PoolStringArray) [], bool p_ssl_validate_domain = (bool) True, HTTPClient::Method p_method = (HTTPClient::Method) 0, String p_request_data = (String) ){
	return Error::from_wasgo_id(_wasgo_HTTPRequest_wrapper_request(wasgo_id, ((Variant) url).get_wasgo_id(), ((Variant) custom_headers).get_wasgo_id(), ssl_validate_domain, ((Variant) method).get_wasgo_id(), ((Variant) request_data).get_wasgo_id()));
}
void HTTPRequest::set_body_size_limit(int p_bytes){
	_wasgo_HTTPRequest_wrapper_set_body_size_limit(wasgo_id, bytes);
}
void HTTPRequest::set_download_file(String p_path){
	_wasgo_HTTPRequest_wrapper_set_download_file(wasgo_id, ((Variant) path).get_wasgo_id());
}
void HTTPRequest::set_max_redirects(int p_amount){
	_wasgo_HTTPRequest_wrapper_set_max_redirects(wasgo_id, amount);
}
void HTTPRequest::set_use_threads(bool p_enable){
	_wasgo_HTTPRequest_wrapper_set_use_threads(wasgo_id, enable);
}

HTTPRequest::HTTPRequest(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
