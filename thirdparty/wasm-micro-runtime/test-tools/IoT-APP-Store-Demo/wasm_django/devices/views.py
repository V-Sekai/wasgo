...
def apps(req):
    ...
    if msg != "":
        if msg.decode() == "fail":
            ...
            for i in range(app_num):
                app_list.append({"pname": dic["applet" + str(i + 1)], "status": "Installed", "current_version": "1.0"})

    alist = app_list
    device_info = []
    device_info.append({"IP": dev_addr, "Port": str(dev_port), "apps": app_num})
    ...
    return render(
        ...
        "application.html",
        {
            ...
            "dlist": json.dumps(device_info),
            "llist": json.dumps(avaliable_list),
            "open_status": json.dumps(open_status),
            "search_node": json.dumps(search_node),
        },
    )
...
def appDownload(req):
    ...
    try:
        ...
        s.send(
            bytes(
                "install:" + dev_addr + ":" + str(dev_port) + ":" + app_name + ":" + app_path + app_name + ".wasm",
                encoding="utf8",
            )
        )
        msg = s.recv(1024)
    ...
    if msg == b"fail":
        ...
        return HttpResponse(json.dumps({"status": eval(msg.decode())["error message"].split(":")[1]}))
...
def appDelete(req):
    ...
    port = 8889
    s.connect((host, port))
    s.send(bytes("uninstall:" + dev_addr + ":" + str(dev_port) + ":" + app_name, encoding="utf8"))
    msg = s.recv(1024)
    s.close()
    r = HttpResponse("ok")
    return r
...
static_list = [
    ...
    {"ID": "event_publisher", "Version": "3.0"},
    {"ID": "event_subscriber", "Version": "1.0"},
    {"ID": "reuqest_handler", "Version": "1.0"},
    {"ID": "sensor", "Version": "1.0"},
    {"ID": "ui_app", "Version": "1.0"},
]
...
def store(req):
    ...
    return render(
        req,
        "appstore.html",
        {"staticlist": json.dumps(static_list), "flist": json.dumps(user_file_list), "ulist": json.dumps(status)},
    )
