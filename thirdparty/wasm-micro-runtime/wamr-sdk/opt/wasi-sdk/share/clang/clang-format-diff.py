...
def main():
    ...
    parser.add_argument(
        "-iregex",
        metavar="PATTERN",
        default=r".*\.(cpp|cc|c\+\+|cxx|c|cl|h|hh|hpp|m|mm|inc|js|ts|proto" r"|protodevel|java|cs)",
        help="custom pattern selecting file paths to reformat " "(case insensitive, overridden by -regex)",
    )
    parser.add_argument(
        "-sort-includes", action="store_true", default=False, help="let clang-format sort include blocks"
    )
    ...
    for filename, lines in lines_by_file.items():
        ...
        p = subprocess.Popen(
            command, stdout=subprocess.PIPE, stderr=None, stdin=subprocess.PIPE, universal_newlines=True
        )
        ...
        if not args.i:
            ...
            diff = difflib.unified_diff(
                code, formatted_code, filename, filename, "(before formatting)", "(after formatting)"
            )
