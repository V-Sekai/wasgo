...
Variant FuncRef::call_funcv(Array p_arg_array){
	return Variant::from_wasgo_id(_wasgo_FuncRef_wrapper_call_funcv(wasgo_id, ((Variant) arg_array).get_wasgo_id()));
}
...
void FuncRef::set_function(String p_name){
	_wasgo_FuncRef_wrapper_set_function(wasgo_id, ((Variant) name).get_wasgo_id());
}
void FuncRef::set_instance(Object p_instance){
	_wasgo_FuncRef_wrapper_set_instance(wasgo_id, ((Variant) instance).get_wasgo_id());
}

FuncRef::FuncRef(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
FuncRef::~FuncRef(){
}