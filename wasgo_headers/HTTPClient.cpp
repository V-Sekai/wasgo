...
Error HTTPClient::connect_to_host(String p_host, int p_port = (int) -1, bool p_use_ssl = (bool) False, bool p_verify_host = (bool) True){
	return Error::from_wasgo_id(_wasgo_HTTPClient_wrapper_connect_to_host(wasgo_id, ((Variant) host).get_wasgo_id(), port, use_ssl, verify_host));
}
...
String HTTPClient::query_string_from_dict(Dictionary p_fields){
	return String::from_wasgo_id(_wasgo_HTTPClient_wrapper_query_string_from_dict(wasgo_id, ((Variant) fields).get_wasgo_id()));
}
...
Error HTTPClient::request(HTTPClient::Method p_method, String p_url, PoolStringArray p_headers, String p_body = (String) ){
	return Error::from_wasgo_id(_wasgo_HTTPClient_wrapper_request(wasgo_id, ((Variant) method).get_wasgo_id(), ((Variant) url).get_wasgo_id(), ((Variant) headers).get_wasgo_id(), ((Variant) body).get_wasgo_id()));
}
Error HTTPClient::request_raw(HTTPClient::Method p_method, String p_url, PoolStringArray p_headers, PoolByteArray p_body){
	return Error::from_wasgo_id(_wasgo_HTTPClient_wrapper_request_raw(wasgo_id, ((Variant) method).get_wasgo_id(), ((Variant) url).get_wasgo_id(), ((Variant) headers).get_wasgo_id(), ((Variant) body).get_wasgo_id()));
}
void HTTPClient::set_blocking_mode(bool p_enabled){
	_wasgo_HTTPClient_wrapper_set_blocking_mode(wasgo_id, enabled);
}
void HTTPClient::set_connection(StreamPeer p_connection){
	_wasgo_HTTPClient_wrapper_set_connection(wasgo_id, ((Variant) connection).get_wasgo_id());
}
void HTTPClient::set_read_chunk_size(int p_bytes){
	_wasgo_HTTPClient_wrapper_set_read_chunk_size(wasgo_id, bytes);
}

HTTPClient::HTTPClient(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
