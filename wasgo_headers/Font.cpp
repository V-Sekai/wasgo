...
void Font::draw(RID p_canvas_item, Vector2 p_position, String p_string, Color p_modulate = Color(1,1,1,1), int p_clip_w = (int) -1, Color p_outline_modulate = Color(1,1,1,1)){
	_wasgo_Font_wrapper_draw(wasgo_id, ((Variant) p_canvas_item).get_wasgo_id(), ((Variant) p_position).get_wasgo_id(), ((Variant) p_string).get_wasgo_id(), ((Variant) p_modulate).get_wasgo_id(), p_clip_w, ((Variant) p_outline_modulate).get_wasgo_id());
}
float Font::draw_char(RID p_canvas_item, Vector2 p_position, int p_char, int p_next = (int) -1, Color p_modulate = Color(1,1,1,1), bool p_outline = (bool) false){
	return (float) _wasgo_Font_wrapper_draw_char(wasgo_id, ((Variant) p_canvas_item).get_wasgo_id(), ((Variant) p_position).get_wasgo_id(), p_char, p_next, ((Variant) p_modulate).get_wasgo_id(), p_outline);
}
...
Vector2 Font::get_char_size(int p_char, int p_next = (int) 0){
	return Vector2::from_wasgo_id(_wasgo_Font_wrapper_get_char_size(wasgo_id, p_char, p_next));
}
...
Vector2 Font::get_string_size(String p_string){
	return Vector2::from_wasgo_id(_wasgo_Font_wrapper_get_string_size(wasgo_id, ((Variant) p_string).get_wasgo_id()));
}
Vector2 Font::get_wordwrap_string_size(String p_string, float p_width){
	return Vector2::from_wasgo_id(_wasgo_Font_wrapper_get_wordwrap_string_size(wasgo_id, ((Variant) p_string).get_wasgo_id(), p_width));
}
