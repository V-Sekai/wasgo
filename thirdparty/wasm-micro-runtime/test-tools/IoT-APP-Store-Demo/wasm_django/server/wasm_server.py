...
def decode_attr_container(msg):
    ...
    logging.info(
        "total_len:{}, tag_len:{}, tag_name:{}, attr_num:{}".format(
            str(total_len), str(tag_len), str(tag_name), str(attr_num)
        )
    )
            ...
            if s is self.server:
                ...
                if data != b"":
                    ...
                    if data.decode().split(":")[0] == "query":
                        ...
                        if client_addr in self.conn_dict.keys():
                            print(
                                'start install application %s to ("%s":"%s")'
                                % (app_name, client_addr[0], client_addr[1])
                            )
