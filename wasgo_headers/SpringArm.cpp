...
void SpringArm::add_excluded_object(RID p_RID){
	_wasgo_SpringArm_wrapper_add_excluded_object(wasgo_id, ((Variant) RID).get_wasgo_id());
}
...
bool SpringArm::remove_excluded_object(RID p_RID){
	return (bool) _wasgo_SpringArm_wrapper_remove_excluded_object(wasgo_id, ((Variant) RID).get_wasgo_id()));
}
void SpringArm::set_collision_mask(int p_mask){
	_wasgo_SpringArm_wrapper_set_collision_mask(wasgo_id, mask);
}
void SpringArm::set_length(float p_length){
	_wasgo_SpringArm_wrapper_set_length(wasgo_id, length);
}
void SpringArm::set_margin(float p_margin){
	_wasgo_SpringArm_wrapper_set_margin(wasgo_id, margin);
}
void SpringArm::set_shape(Shape p_shape){
	_wasgo_SpringArm_wrapper_set_shape(wasgo_id, ((Variant) shape).get_wasgo_id());
}

SpringArm::SpringArm(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
SpringArm::~SpringArm(){
}