...
Error MultiplayerAPI::send_bytes(PoolByteArray p_bytes, int p_id = (int) 0, NetworkedMultiplayerPeer::TransferMode p_mode = (NetworkedMultiplayerPeer::TransferMode) 2){
	return Error::from_wasgo_id(_wasgo_MultiplayerAPI_wrapper_send_bytes(wasgo_id, ((Variant) p_bytes).get_wasgo_id(), p_id, ((Variant) p_mode).get_wasgo_id()));
}
void MultiplayerAPI::set_root_node(Node p_node){
	_wasgo_MultiplayerAPI_wrapper_set_root_node(wasgo_id, ((Variant) p_node).get_wasgo_id());
}