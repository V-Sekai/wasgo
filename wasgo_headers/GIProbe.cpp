...
void GIProbe::bake(Node p_from_node = (Node) "", bool p_create_visual_debug = (bool) False){
	_wasgo_GIProbe_wrapper_bake(wasgo_id, ((Variant) from_node).get_wasgo_id(), create_visual_debug);
}
...
void GIProbe::set_bias(float p_max){
	_wasgo_GIProbe_wrapper_set_bias(wasgo_id, max);
}
void GIProbe::set_compress(bool p_enable){
	_wasgo_GIProbe_wrapper_set_compress(wasgo_id, enable);
}
void GIProbe::set_dynamic_range(int p_max){
	_wasgo_GIProbe_wrapper_set_dynamic_range(wasgo_id, max);
}
void GIProbe::set_energy(float p_max){
	_wasgo_GIProbe_wrapper_set_energy(wasgo_id, max);
}
void GIProbe::set_extents(Vector3 p_extents){
	_wasgo_GIProbe_wrapper_set_extents(wasgo_id, ((Variant) extents).get_wasgo_id());
}
void GIProbe::set_interior(bool p_enable){
	_wasgo_GIProbe_wrapper_set_interior(wasgo_id, enable);
}
void GIProbe::set_normal_bias(float p_max){
	_wasgo_GIProbe_wrapper_set_normal_bias(wasgo_id, max);
}
void GIProbe::set_probe_data(GIProbeData p_data){
	_wasgo_GIProbe_wrapper_set_probe_data(wasgo_id, ((Variant) data).get_wasgo_id());
}
void GIProbe::set_propagation(float p_max){
	_wasgo_GIProbe_wrapper_set_propagation(wasgo_id, max);
}
void GIProbe::set_subdiv(GIProbe::Subdiv p_subdiv){
	_wasgo_GIProbe_wrapper_set_subdiv(wasgo_id, ((Variant) subdiv).get_wasgo_id());
}

GIProbe::GIProbe(WasGoId p_wasgo_id) : Variant(p_wasgo_id){
}
GIProbe::~GIProbe(){
}